...
async def create_shop(db:Session,shop:ShopSchema ,user:User):
    ...
    new_shop=Shop(name=shop.name,Business_reg_no=shop.Business_reg_no,house_number=shop.house_number,location=shop.location,state=shop.state , street_name=shop.street_name)
    print(user)
